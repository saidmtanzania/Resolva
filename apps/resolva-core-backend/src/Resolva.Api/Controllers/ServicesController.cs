...
public class ServicesController : ControllerBase
{
    ...
    public ServicesController(ResolvaDbContext db) => _db = db;
    ...
    public async Task<ActionResult<CatalogItemResponse>> Create(CreateCatalogItemRequest req)
    {
        ...
        var exists = await _db.Services.AnyAsync(p => p.Name.ToLower() == req.Name.Trim().ToLower());
        if (exists) return Conflict(new { message = "Service name already exists" });
    ...
    {
        var item = await _db.Services.SingleOrDefaultAsync(x => x.Id == id);
